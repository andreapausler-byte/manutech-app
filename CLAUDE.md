...
- **Prossimo step**: lavoro su (a) Interventi v2 / ADR-008 (chiuso in docs 20/5) → escalation a (b) **ADR-007 mig 056 proposal** (20/5 sera tardi). Espansione ADR-007 completata: identificate 27 tabelle reali con `org_id` (lista preliminare ADR aveva 24, 2 errate `dm_reads`/`report_stars`, 4 mancanti `areas`/`assistant_conversations`/`document_chunks`/`machine_components`); stato pre-056 mappato (mig 032 ha già backfillato + DROP DEFAULT solo su 6/27 tabelle, le altre 21 hanno DEFAULT 'default' dormiente); 4 opzioni valutate (A scartata, **C raccomandata**, B differita a sprint dedicato per via di 92 RLS dipendenti, D scartata). Scelta C: TEXT + CHECK constraint UUID-regex + DROP DEFAULT residui — zero modifiche RLS/helper/client. Migration proposal: `supabase/migrations/056_org_id_hardening.sql` (idempotent via `DO $$ FOREACH ... LOOP` + `to_regclass` skip) + DOWN file. **Application richiede human-in-the-loop su Supabase Studio**: review SQL, eseguire, smoke test (`INSERT ... org_id='default'` deve fallire), sanity count `pg_constraint` = 27. Post-application: ADR-007 pivot a `accepted`, ADR-008 sbloccato per pivot a `accepted` (solo Q2/Q4/Q5 residue da confronto manutentore #2). Stato attuale: ADR-008 docs-blocked solo da Q2/Q4/Q5 + ADR-007 mig 056 in produzione. Debt collaterale `role==='fornitore'` nel client resta fuori scope, da chiudere pre-implementation Interventi v2.
