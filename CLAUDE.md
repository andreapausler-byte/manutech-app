...
- **Fase**: **Componenti con documentazione propria** (v5.19) ✓ — la pompa, il motoriduttore e il quadro smettono di essere righe di anagrafica in sola lettura: ognuno tiene i suoi documenti e le sue foto, senza dover diventare un macchinario a sé. I file però NON escono dalla macchina — restano in `machines.attachments` con l'etichetta `component_id` (migration 062), quindi compaiono comunque in Galleria Foto, nelle cartelle documentali e nella biblioteca AI. Il tab Componenti è una lente su quei file, non un secondo archivio. `MachineComponentsTab` è master-detail con Foto / Documento / Archivia esistente; l'AI indicizza anche le schede dei pezzi. Decisione e alternative scartate: ADR-012.
- **Branch corrente**: `claude/machine-components-documents-wwle6b` — parte da `master` (v5.18 + v5.18.1 già mergiati). Le migration 060, 061 e **062** non sono ancora applicate in produzione, e l'edge function `ingest-knowledge` va ridistribuita.
- **Prossimo step**: (a) applicare **060, 061 e 062** in produzione — senza la 060 la galleria resta vuota, senza la 061 Scatta e Carica documento danno errore, senza la 062 i file sul componente vengono rifiutati; (b) il reset globale in `styles/index.css` neutralizza tutte le utility di padding/margin di Tailwind in tutta l'app (vedi Debito tecnico) — va deciso se sistemarlo; (c) componenti dal mobile (oggi sola lettura: `useMachineUpload` accetta già il pezzo, manca la schermata) e scelta del componente mentre si crea una segnalazione; (d) galleria per l'operatore in `OperatorApp`, e open question dell'ADR-011 (bucket privato, retention GDPR, video non compressi); (e) resta aperta la review trimestrale ROADMAP (era in agenda il **2/8**).
