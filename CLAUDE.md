...
- **Fase**: **Il pezzo nel ciclo di vita del ticket** (v5.21) — dopo la v5.19 (componenti con documentazione propria, ADR-012) e la v5.20 (tab Pezzi dal mobile), il componente esce dall'anagrafica ed entra nel lavoro quotidiano: la pastiglia ciano del pezzo compare in **ogni** vista del ticket, tecnico e admin lo attribuiscono e lo correggono dal dettaglio e in chiusura, e i `maintenance_log` lo **ereditano** dal ticket risolto o dal piano confermato — lo storico del pezzo si popola senza data entry. In più `maintenance_plans.component_id` (migration 063): il piano resta della macchina ma può nominare il pezzo. Studio di riferimento: `docs/proposals/2026-08-26-segnalazioni-per-componente.md` (passi A, B, C e §5 attuati).
- **Branch corrente**: `claude/machinery-components-management-1h7kin`. La v5.20 è in `master`. La v5.21 porta **una migration nuova: 063** (`maintenance_plans.component_id` + `create_maintenance_plan` con 7 parametri — la vecchia RPC a 6 viene droppata, altrimenti PostgREST trova due funzioni omonime).
- **Prossimo step**: (a) **§D dello studio** — il vocale (`extract-ticket-fields`) estrae ancora `componente` come testo libero in `extra_data` senza match con `machine_components`: proporre il pezzo in revisione, mai assegnarlo in automatico; (b) **§E** — statistiche dal lato pezzo (MTBF del componente, filtro per componente nel tab Segnalazioni) e `spare_parts.component_id`, che esiste dalla 022 ed è inutilizzato; (c) **§4 — gerarchia impianti**: da decidere **sui dati** che A-C cominciano a produrre adesso, non prima; resta aperta la domanda se in stabilimento esistano impianti di servizio condivisi (centrale frigo, aria compressa, vapore), che non sono né componenti né figli; (d) creazione componenti: **decisa admin-only il 28/8** — se i ticket "generici" restano la maggioranza, sospettare l'anagrafica incompleta prima della UI; (e) il reset globale in `styles/index.css` neutralizza le utility di padding/margin in tutta l'app (vedi Debito tecnico); (f) resta aperta la review trimestrale ROADMAP (era in agenda il **2/8**).
