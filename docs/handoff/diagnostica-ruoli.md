...
**Pattern strutturale scoperto:** convivono **email funzionali di postazione/reparto** (`@amarcord.it`: `ctcantina`, `ctsalacotte`, `ctriempimenti`, `qc`, `manutenzione`, `magazzino`, `tank`) ed **email personali** (gmail/outlook/virgilio). Alcune persone hanno **entrambe** → la duplicazione non è "due email a caso", è **"account-postazione + account-personale"**. Tocca la questione *identità condivisa vs individuale*: `qc@`, `manutenzione@`, `magazzino@` sembrano **caselle di reparto** potenzialmente condivise. ⚠️ **Da confermare con Andrea**: se alcune email-postazione sono usate da più persone, il principio "identità unica" del design di destinazione va calibrato (una postazione non è una persona).
