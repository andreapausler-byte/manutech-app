  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
        ...
        {unit && <span style={{ fontFamily: fMono, fontSize: 13, color: MT.textMuted }}>{unit}</span>}
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontFamily: fMono, fontSize: 12, color: dColor, letterSpacing: 0.5 }}>
          {dSign}{delta} {delta > 0 ? '▲' : '▼'}
        </span>
        {sub && <span style={{ fontFamily: fMono, fontSize: 11, color: MT.textDim, letterSpacing: 0.5 }}>{sub}</span>}
      </div>
    </div>
  )
}
